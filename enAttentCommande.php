...
$stmt = $con->prepare("SELECT * FROM `orders` WHERE `status` = 'en attente'");
$stmt->execute();
...
td button.confirm { background-color: #27ae60; }
td button.delete { background-color: var(--primary); }
...
<tr data-id="<?= $order['id'] ?>">
    ...
    <td data-label="Actions">
        <button class="confirm"><i class="fas fa-check"></i> Confirmer</button>
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    table.addEventListener('click', e=>{
        ...
        if(btn.classList.contains('confirm')){
            if(confirm('Mettre la commande en "en cours de traitement" ?')){
